...
/* For verification */
int checkResult = 0;
unsigned long long checkResultNodes = 0;
...
double uts_wctime() {
	...
	return (tv.tv_sec * 1E3 + 1E-3 * tv.tv_usec);
}
#endif
...
void uts_parseParams(int argc, char *argv[]){
	...
	while (i < argc && err == -1) {
		...
		switch (argv[i][1]) {
			...
			case 'C':
				checkResult = 1;
				checkResultNodes = atol(argv[i+1]);
				break;
			default:
				err = i;
		}
...
void uts_showStats(int nPes, int chunkSize, double walltime, counter_t nNodes, counter_t nLeaves, counter_t maxDepth) {
	if (checkResult) {
		if (nNodes != checkResultNodes) {
			printf("!!! FAILURE: Counted %llu nodes, expected %llu nodes\n",
				    nNodes, checkResultNodes);
		} else {
			printf("!!! SUCCESS\n");
		}
	}
	...
	// summarize execution info for human consumption
		...
		printf("Elapsed wall time: %.3f ms, performance = %.0f nodes/ms (%.0f nodes/ms per PE)\n\n",
